...
const StepSecond = () => {
  ...
  const AddFields = (e, val, ind) => {
    ...
    if (ind === 0) {
      ...
      const AN = "Asset_name" + updatedList[updatedList.length-1];
      const CN = "Custodion_Name" + updatedList[updatedList.length-1];
      const WN = "Wallet_Address" + updatedList[updatedList.length-1];
